...
void WJetsYields(){
  ...
  */
  ...
  TFile *f1 = TFile::Open("newDMF_newTightIso_LooseElectron_TightMuon/SR/W+Jets.root","READ");
  if (f1 == 0) {
  ...
  TFile *f2 = TFile::Open("newDMF_newTightIso_LooseElectron_TightMuon_TAUPOG_NewTriggerSF/SR/W+Jets.root","READ");
  if (f2 == 0) {
    printf("Error: cannot open DY+Jets \n");
    return;
    }
  ...
  //--------------FIRSTPAD
  H_Mass_3000->SetTitle("Comparison no Trigger-Trigger on MC for WJets");
